...
def _build_health_app(session_factory: async_sessionmaker) -> FastAPI:
    app = FastAPI()
    app.include_router(health.router)
    app.state.settings = SimpleNamespace(
        qdrant_url="http://qdrant.invalid",
        minio_url="http://minio.invalid",
    )
...
async def test_ready_returns_failure_when_database_is_unreachable(
    ...
    transport = httpx.ASGITransport(app=_build_health_app(failing_session_factory))  # type: ignore[arg-type]
    async with httpx.AsyncClient(transport=transport, base_url="http://testserver") as client:
        response = await client.get("/ready")
