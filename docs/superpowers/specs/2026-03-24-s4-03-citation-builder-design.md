...
**Note:** The story quote uses `[source_id:N]` — the original wording from `docs/plan.md`. Per D1 below, the implemented marker format is `[source:N]` (ordinal index, not UUID-based). All implementation follows D1. The plan.md wording is kept as-is for traceability; `docs/spec.md` and `docs/rag.md` are updated as part of this story (see Files Changed).
...
+ section: ", {anchor_section}"            (if non-null and chapter absent)
...
- `"Design Patterns", Section "Observer"` — DOCX with section heading
