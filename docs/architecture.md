...
4. Worker determines the processing path:
   - **Path A (Gemini direct)** — for native multimodal inputs within limits: images, audio ≤ 80 sec, video ≤ 120 sec, and short PDFs only when the router explicitly prefers Gemini-native handling. Text-based PDFs remain on Path B by default even when they are short. Worker creates a single chunk record for the entire file. Two separate Gemini calls:
