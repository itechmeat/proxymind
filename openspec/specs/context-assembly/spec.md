...
- **WHEN** adding another parent-bearing prompt unit would exceed the retrieval context budget
- **THEN** the assembler SHALL prefer keeping already-selected matched child evidence
- **AND** it MAY omit lower-priority additional parent-bearing prompt units according to the hierarchy-aware budget policy
