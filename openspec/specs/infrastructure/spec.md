...
Docker Compose SHALL define five services: `postgres`, `qdrant`, `minio`, `redis`, and `api`. Each service SHALL use image versions at or above the minimums specified in `docs/spec.md`. The `api` service SHALL build from `./backend` and expose port 8000. No `worker` service is included at this stage (deferred to S2-01).
...
- **THEN** all five services SHALL reach a running state without errors
