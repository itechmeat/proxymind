...
The system SHALL expose `GET /api/chat/twin` as a public endpoint (no authentication required). The endpoint SHALL return a JSON object with `name` (string) and `has_avatar` (boolean) fields, sourced from the `agents` table for `DEFAULT_AGENT_ID`. The `has_avatar` field SHALL be derived from `avatar_url IS NOT NULL`. The response SHALL NOT expose any internal URLs or storage keys.

#### Scenario: Agent exists with avatar

- **WHEN** `GET /api/chat/twin` is called and the agent has a non-null `avatar_url`
- **THEN** the response SHALL be 200 with `{ "name": "<agent name>", "has_avatar": true }`

#### Scenario: Agent exists without avatar

- **WHEN** `GET /api/chat/twin` is called and the agent has `avatar_url = NULL`
- **THEN** the response SHALL be 200 with `{ "name": "<agent name>", "has_avatar": false }`

#### Scenario: Agent not found

- **WHEN** `GET /api/chat/twin` is called and no agent exists for `DEFAULT_AGENT_ID`
...
The system SHALL expose `GET /api/chat/twin/avatar` as a public endpoint that proxies the avatar image from SeaweedFS. The endpoint SHALL return the image bytes with the correct `Content-Type` header derived from the file extension. The browser SHALL NOT communicate with SeaweedFS directly.

#### Scenario: Avatar exists

- **WHEN** `GET /api/chat/twin/avatar` is called and the agent has a non-null `avatar_url`
...
- **WHEN** `GET /api/chat/twin/avatar` is called and the agent has `avatar_url = NULL`
- **THEN** the response SHALL be 404
