...
- **Docker Compose services count test**: verify `docker compose config --services` lists all 9 runtime services (excluding `backend-test`).
- **Named volumes test**: verify the `volumes` section of `docker-compose.yml` includes all 7 named volumes.
- **Configuration files exist test**: verify all 6 configuration files exist at expected paths.

### Evals (non-deterministic, manual verification)

- **Docker Compose smoke test**: `docker compose up` starts all 9 runtime services without errors.
- **Prometheus scrape target up**: Prometheus targets page shows `proxymind-api` as UP.
- **Grafana dashboard visible**: ProxyMind Overview dashboard loads in Grafana.
