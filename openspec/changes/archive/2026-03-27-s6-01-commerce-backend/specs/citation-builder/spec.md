...
Citation marker extraction from LLM output, source metadata resolution (URL/text), text citation formatting, prompt citation instructions, and citation object structure. Introduced by S4-03. Extended by S6-01 with purchase link fields on SourceInfo and Citation for catalog item enrichment.
...
- `purchase_url` — str or null, purchase/store URL from linked catalog item (null when no active catalog item)
- `purchase_title` — str or null, name of the linked catalog item (null when no active catalog item)
- `catalog_item_type` — str or null, type of the linked catalog item: book/course/event/merch/other (null when no active catalog item)

#### Scenario: Citation object with URL and purchase enrichment

- **WHEN** a citation is built for a source with title "Clean Architecture", source_type "pdf", public_url "https://example.com/article", linked to an active catalog item with url "https://store.com/clean-arch", name "Clean Architecture Book", type "book"
- **THEN** the citation object SHALL have `url = "https://example.com/article"`, `purchase_url = "https://store.com/clean-arch"`, `purchase_title = "Clean Architecture Book"`, `catalog_item_type = "book"`

#### Scenario: Citation object without URL and without purchase

- **WHEN** a citation is built for a source with title "Clean Architecture", source_type "pdf", public_url null, no linked catalog item, chapter "Chapter 5", page 42
- **THEN** the citation object SHALL have `url = null`, `purchase_url = null`, `purchase_title = null`, `catalog_item_type = null`
- **AND** `text_citation` SHALL be `"Clean Architecture", Chapter 5, p. 42`

#### Scenario: Citation object with URL but no purchase

- **WHEN** a citation is built for a source with public_url "https://docs.python.org", no linked catalog item
- **THEN** `url` SHALL be "https://docs.python.org"
- **AND** `purchase_url`, `purchase_title`, `catalog_item_type` SHALL all be `null`

---

### Requirement: Source metadata loading

The system SHALL batch-load source metadata (title, public_url, source_type) from PostgreSQL by unique `source_id` values extracted from retrieved chunks. The query SHALL include only non-deleted sources (`deleted_at IS NULL`). The query SHALL LEFT JOIN `catalog_items` to include catalog item metadata when a source has a linked catalog item. The result SHALL be a `source_map` dict mapping `UUID` to `SourceInfo`. The `SourceInfo` dataclass SHALL include catalog item fields: `catalog_item_url`, `catalog_item_name`, `catalog_item_type`, `catalog_item_active`. The batch load SHALL execute once after retrieval, and the resulting `source_map` SHALL be reused by both the prompt builder and the citation service.
...
#### Scenario: Source map includes catalog item data

- **WHEN** source A has `catalog_item_id` pointing to an active catalog item with url, name, and type
- **THEN** `source_map[A].catalog_item_url` SHALL be the catalog item's URL
- **AND** `source_map[A].catalog_item_name` SHALL be the catalog item's name
- **AND** `source_map[A].catalog_item_type` SHALL be the catalog item's type string
- **AND** `source_map[A].catalog_item_active` SHALL be `True`

#### Scenario: Source without catalog link has default catalog fields

- **WHEN** source C has `catalog_item_id = NULL`
- **THEN** `source_map[C].catalog_item_url` SHALL be `None`
- **AND** `source_map[C].catalog_item_name` SHALL be `None`
- **AND** `source_map[C].catalog_item_type` SHALL be `None`
- **AND** `source_map[C].catalog_item_active` SHALL be `False`
...
- **WHEN** a citation is extracted for a source whose `SourceInfo` has `catalog_item_active=True` and `catalog_item_url="https://store.com/book"`
- **THEN** the citation SHALL have `purchase_url = "https://store.com/book"`, `purchase_title` from the catalog item name, `catalog_item_type` from the catalog item type

#### Scenario: No enrichment for inactive catalog item

- **WHEN** a citation is extracted for a source whose `SourceInfo` has `catalog_item_active=False`
- **THEN** `purchase_url`, `purchase_title`, and `catalog_item_type` SHALL all be `None`

#### Scenario: No enrichment when no catalog item linked

- **WHEN** a citation is extracted for a source with no linked catalog item (default `SourceInfo` values)
- **THEN** `purchase_url`, `purchase_title`, and `catalog_item_type` SHALL all be `None`

#### Scenario: Active catalog item with null URL

- **WHEN** a citation is extracted for a source whose `SourceInfo` has `catalog_item_active=True` but `catalog_item_url=None`
- **THEN** `purchase_url` SHALL be `None`
- **AND** `purchase_title` SHALL still be populated from the catalog item name
- **AND** `catalog_item_type` SHALL still be populated

---

### Requirement: Citation persistence with purchase fields

Resolved citations SHALL be stored in the `Message.citations` JSONB field as an array of citation objects. Each citation object in the array SHALL conform to the extended citation object structure including `purchase_url`, `purchase_title`, and `catalog_item_type` fields. The `to_dict()` method SHALL include these fields. The `from_dict()` method SHALL read these fields (defaulting to `None` for backward compatibility with pre-S6-01 citations).

#### Scenario: Enriched citations persisted to JSONB

- **WHEN** the LLM output produces 2 resolved citations, one with purchase enrichment and one without
- **THEN** `Message.citations` SHALL contain a JSON array with 2 citation objects
- **AND** the enriched citation SHALL include non-null `purchase_url`, `purchase_title`, `catalog_item_type`
- **AND** the non-enriched citation SHALL include null values for those fields

#### Scenario: Pre-S6-01 citations deserialized safely

- **WHEN** `from_dict()` is called on a citation dict that does not contain `purchase_url`, `purchase_title`, `catalog_item_type` keys
- **THEN** those fields SHALL default to `None`
